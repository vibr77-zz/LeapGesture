...
- (void)addTouches:(NSSet*)set 
{
	//[self addTouchAtPoint:[[set anyObject]]];
}
- (void)addTouchAtPoint:(CGPoint)point
{
	[touchPoints addObject:[NSValue valueWithPoint:point]];
}
- (long)CountTouchAtPoint
{
...
- (NSString *)findBestMatchCenter:(CGPoint*)outCenter angle:(float*)outRadians score:(float*)outScore
{
	@autoreleasepool {
		...
		{
			samples[i] = [[[self touchPoints] objectAtIndex:MAX(0, (c-1)*i/(samplePoints-1))] pointValue];
		}
		...
		//Rotate(samples, samplePoints, -firstPointAngle); // Removed By V
		...
		for (NSString *templateName in [templates allKeys])
		{
			...
			for (i = 0; i < samplePoints; i++)
			{
				template[i] = [[templateSamples objectAtIndex:i] pointValue];
			}
		...
		{
			CGPoint pt = samples[i];
			[resampledPoints addObject:[NSValue valueWithPoint:pt]];
		}
